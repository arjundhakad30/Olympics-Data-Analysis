...
def most_successful(df, sport):
    ...
    # Check if 'region' exists in df before selecting it
    available_columns = ['Name', 'Medal', 'Sport']
    if 'region' in top_athletes.columns:
        available_columns.append('region')

    # Select available columns and drop duplicates by Name
    top_athletes = top_athletes[available_columns].drop_duplicates('Name')
...
def most_successful_countrywise(df, country):
    ...
    # Remove rows with no medal (optional, depending on your needs)
    temp_df = temp_df[temp_df['Medal'].notna()]

    # Count medals per athlete and create a DataFrame
    top_athletes = temp_df['Name'].value_counts().reset_index()
    top_athletes.columns = ['Name', 'Medal_Count']

    # Merge with original df to get athlete details (e.g. Sport)
    merged_df = top_athletes.head(10).merge(df, on='Name', how='left')[['Name', 'Medal_Count', 'Sport', 'region']].drop_duplicates('Name')
